"""
Cross-Domain Evaluation — FL Global (Augmented) Only (re-uses cached baseline numbers)
====================================================================================
Baselines (COD10K-only, Kvasir-only) numbers are hard-coded below from the
previous full run. This script re-evaluates the FL Global checkpoint (from 
checkpoints/fl_checkpoints_aug_new/global_best.pth) on both test sets and merges it with cached
baseline numbers to reproduce the full comparison table.

Kvasir test set definition:
  Matches train/fed_train_aug.py exactly. The Kvasir split is stem-grouped and
  persisted in kvasir_split.json — built from original (pre-augmentation) stems.

LOCAL STRUCTURE:
  ./
  ├── COD10K-v3/
  ├── Kvasir-SEG-aug/
  └── checkpoints/fl_checkpoints_aug_new/
      ├── kvasir_split.json
      └── global_best.pth
...
KVASIR_IMAGE_DIR   = str(PROJECT_ROOT / "Kvasir-SEG-aug" / "images")
KVASIR_MASK_DIR    = str(PROJECT_ROOT / "Kvasir-SEG-aug" / "masks")

# Suffixes used in fed_train_aug.py to recover the original stem
KV_AUG_SUFFIXES = ("_hflip", "_rotate", "_colour")

# Points at the checkpoint directory from augmented federated training
FL_CKPT_DIR    = str(PROJECT_ROOT / "checkpoints" / "fl_checkpoints_aug_new")
KV_SPLIT_FILE  = os.path.join(FL_CKPT_DIR, "kvasir_split.json")
FL_CKPT        = os.path.join(FL_CKPT_DIR, "global_best.pth")

RESULTS_PATH   = str(PROJECT_ROOT / "results" / "cross_domain_results_fl_aug.json")
...
# ─────────────────────────────────────────────────────
# CACHED BASELINE RESULTS — from earlier full run.
# ─────────────────────────────────────────────────────
...
def build_kvasir_test_records() -> list:
    if not os.path.exists(KV_SPLIT_FILE):
        raise FileNotFoundError(
            ...
            f"Run train/fed_train_aug.py at least once first (it creates this file)."
        )
...
def print_results_table(all_results: dict):
    ...
    print(f"\n{'='*80}")
    print(f"  CROSS-DOMAIN EVALUATION — FULL RESULTS TABLE (FL Augmented)")
    print(f"{'='*80}")
    ...
    for model_name in models:
        ...
        tag = "" if model_name != "FL Global" else "  (Aug FL)"
...
def main():
    required = [
        ...
        (KV_SPLIT_FILE,      "Kvasir split file (from train/fed_train_aug.py)"),
    ...
    print(f"\n{'='*80}")
    print(f"  FL-Augmented Cross-Domain Evaluation — COD10K + Kvasir-SEG")
    print(f"{'='*80}\n")
    ...
    all_results = dict(CACHED_BASELINE_RESULTS)
